...
{
	switch(cmd_id){
		case LED_OPEN: 		set_rgb(254,254,254);	
					break;
		case LED_CLOSE: 	set_rgb(0,0,0);		
					break;
		case COLOR_RED:   if((WS2812_RGB.red==0)&&(WS2812_RGB.green==0)&&(WS2812_RGB.blue==0)){
					break;
				  }
				  set_rgb(254,0,0);	
				 	break;

		case COLOR_GREE:  if(WS2812_RGB.red==0  && WS2812_RGB.green==0 && WS2812_RGB.blue==0){
				 	break;
				  }	
				  set_rgb(0,254,0);
				  	break;
		
		case COLOR_BULE:  if(WS2812_RGB.red==0  && WS2812_RGB.green==0 && WS2812_RGB.blue==0){
					break;
		...
		case COLOR_ORANGE:if(WS2812_RGB.red==0 && WS2812_RGB.green==0 && WS2812_RGB.blue==0){
		...
		case COLOR_PURPLE:if(WS2812_RGB.red==0 && WS2812_RGB.green==0 && WS2812_RGB.blue==0){
...
void Task_ParseJSON(void *pvParameters)
{
	...
	while (1)
	{
		...
		else uart_write_bytes(UART_NUM1,"AT+PLAY=21\r\n",13);//播放：已经打开灯了

		if(WS2812_RGB.red!=254&&WS2812_RGB.green!=254&&WS2812_RGB.blue!=254)goto __cJSON_Delete;
		else uart_write_bytes(UART_NUM1,"AT+PLAY=22\r\n",13);//播放：已经关灯了
